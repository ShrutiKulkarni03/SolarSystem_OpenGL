...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		...
		{
			LMBpressed = true;
			winXCoord = GET_X_LPARAM(lParam);
			winYCoord = GET_Y_LPARAM(lParam);
	...
	case WM_MOUSEMOVE:
		...
		case MK_RBUTTON:
			if (frameCount >= 2800)
			{
				...
				mouseInput(winXCoord, winYCoord);
			}
			break;
		}
		break;
...
void Initialize(void)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, STARFIELD_FBO_WIDTH, STARFIELD_FBO_HEIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	...
	glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, STARFIELD_FBO_WIDTH, STARFIELD_FBO_HEIGHT);
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, rbo_starfield);

	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
	{
		fprintf(gpFile, "SF Framebuffer incomplete!\n");

		glDeleteFramebuffers(1, &fbo_starfield);
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
	}

	fprintf(gpFile, "SF Framebuffer created successfully\n");
	glBindTexture(GL_TEXTURE_2D, 0);
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	...
	//create texture object for primitive info buffer
	glGenTextures(1, &m_pickingTexture);
	glBindTexture(GL_TEXTURE_2D, m_pickingTexture);
	//glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, 1920, 1080, 0, GL_RGB, GL_FLOAT, NULL);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, PICKING_FBO_WIDTH, PICKING_FBO_HEIGHT, 0, GL_RGB, GL_FLOAT, NULL);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, PICKING_FBO_WIDTH, PICKING_FBO_HEIGHT, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
	...
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
	{
		fprintf(gpFile, "Framebuffer incomplete!\n");

		glDeleteFramebuffers(1, &m_fbo);
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
	}

	fprintf(gpFile, "Framebuffer created successfully\n");
	...
	//NOISE
	make3DNoiseTexture();
	noise3DTexture = init3DNoiseTexture(Noise3DTexSize, Noise3DTexPtr);
...
void Display(void)
{
	...
	glViewport(0, 0, (GLsizei)STARFIELD_FBO_WIDTH, (GLsizei)STARFIELD_FBO_HEIGHT);
	//glViewport(0, 0, (GLsizei)PICKING_FBO_WIDTH, (GLsizei)PICKING_FBO_HEIGHT);
	...
	perspectiveProjectionMatrix = perspective(50.0f, (GLfloat)gWidth / (GLfloat)gHeight, 0.1f, 1000.0f);
	glViewport(0, 0, (GLsizei)gWidth, (GLsizei)gHeight);
	RenderText(textShaderProgramObject, "not-to-scale", 0.66f, -0.45f, 0.00025f, vec4(1.0f, 1.0f, 1.0f, 1.0f));
	
	if (frameCount >= 2800)
	{
		RenderText(textShaderProgramObject, "NAVIGATION: Arrow Keys", -0.8f, 0.43f, 0.00025f, vec4(1.0f, 1.0f, 1.0f, 1.0f));
		RenderText(textShaderProgramObject, "INFO: Numpad 1-9 OR", -0.8f, 0.41f, 0.00025f, vec4(1.0f, 1.0f, 1.0f, 1.0f));
		RenderText(textShaderProgramObject, "Mouse Click On Planet", -0.8f, 0.39f, 0.00025f, vec4(1.0f, 1.0f, 1.0f, 1.0f));
		RenderText(textShaderProgramObject, "RESET: Key 'R'", -0.8f, 0.37f, 0.00025f, vec4(1.0f, 1.0f, 1.0f, 1.0f));
	}
	...
	if (LMBpressed == true)
	{
		//MessageBox(ghwnd, TEXT("in LMB block"), TEXT("LMB"), MB_OK);

		PixelInfo pix = ReadPixel(winXCoord, (PICKING_FBO_HEIGHT - winYCoord - 1));

		fprintf(gpFile, "oID = %f, dID = %f, pID = %f\n", pix.ObjectID, pix.DrawID, pix.PrimID);

		if (pix.PrimID != 0.0f)
		{
			if (pix.ObjectID == MERCURY_OBJECT_ID)
			{
				bTextViewport = true;
				isMercuryClicked = true;
				isVenusClicked = false;
				...
				isNeptuneClicked = false;
				isPlutoClicked = false;
				//MessageBox(ghwnd, TEXT("MERCURY"), TEXT("LMB"), MB_OK);
			}
			else if (pix.ObjectID == VENUS_OBJECT_ID)
			{
				bTextViewport = true;
				isMercuryClicked = false;
				isVenusClicked = true;
				...
				isNeptuneClicked = false;
				isPlutoClicked = false;
				//MessageBox(ghwnd, TEXT("VENUS"), TEXT("LMB"), MB_OK);
			}
			else if (pix.ObjectID == EARTH_OBJECT_ID)
			{
				bTextViewport = true;
				isMercuryClicked = false;
				isVenusClicked = false;
				...
				isNeptuneClicked = false;
				isPlutoClicked = false;
				//MessageBox(ghwnd, TEXT("EARTH"), TEXT("LMB"), MB_OK);
			}
			else if (pix.ObjectID == MARS_OBJECT_ID)
			{
				bTextViewport = true;
				isMercuryClicked = false;
				isVenusClicked = false;
				...
				isNeptuneClicked = false;
				isPlutoClicked = false;
				//MessageBox(ghwnd, TEXT("MARS"), TEXT("LMB"), MB_OK);
			}
			else if (pix.ObjectID == JUPITER_OBJECT_ID)
			{
				bTextViewport = true;
				isMercuryClicked = false;
				isVenusClicked = false;
				...
				isNeptuneClicked = false;
				isPlutoClicked = false;
				//MessageBox(ghwnd, TEXT("JUPITER"), TEXT("LMB"), MB_OK);
			}
			else if (pix.ObjectID == SATURN_OBJECT_ID)
			{
				bTextViewport = true;
				isMercuryClicked = false;
				isVenusClicked = false;
				...
				isNeptuneClicked = false;
				isPlutoClicked = false;
				//MessageBox(ghwnd, TEXT("SATURN"), TEXT("LMB"), MB_OK);
			}
			else if (pix.ObjectID == URANUS_OBJECT_ID)
			{
				bTextViewport = true;
				isMercuryClicked = false;
				isVenusClicked = false;
				...
				isNeptuneClicked = false;
				isPlutoClicked = false;
				//MessageBox(ghwnd, TEXT("URANUS"), TEXT("LMB"), MB_OK);
			}
			else if (pix.ObjectID == NEPTUNE_OBJECT_ID)
			{
				bTextViewport = true;
				isMercuryClicked = false;
				isVenusClicked = false;
				...
				isNeptuneClicked = true;
				isPlutoClicked = false;
				//MessageBox(ghwnd, TEXT("NEPTUNE"), TEXT("LMB"), MB_OK);
			}
			else if (pix.ObjectID == PLUTO_OBJECT_ID)
			{
				bTextViewport = true;
				isMercuryClicked = false;
				isVenusClicked = false;
				...
				isNeptuneClicked = false;
				isPlutoClicked = true;
				//MessageBox(ghwnd, TEXT("PLUTO"), TEXT("LMB"), MB_OK);
			}
			else
			{
				bTextViewport = false;
				...
				isNeptuneClicked = false;
				isPlutoClicked = false;
				//MessageBox(ghwnd, TEXT("NONE"), TEXT("LMB"), MB_OK);
			}
		}
		else
		{
			...
			isNeptuneClicked = false;
			isPlutoClicked = false;
			//MessageBox(ghwnd, TEXT("BG"), TEXT("LMB"), MB_OK);
		}
...
void Update(void)
{
	...
	{
		jupiterRevAngle += 0.0008f;
		if (jupiterRevAngle >= 2 * PI)
			jupiterRevAngle = 0.0f;

		jupiterRotAngle -= 0.5f;
	...
	{
		neptuneRevAngle -= 0.0004f;
		if (neptuneRevAngle <= -2 * PI)
			neptuneRevAngle = 0.0f;

		neptuneRotAngle -= 0.5f;
